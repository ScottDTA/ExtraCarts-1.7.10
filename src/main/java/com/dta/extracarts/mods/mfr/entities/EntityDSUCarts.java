...
public class EntityDSUCarts extends EntityExtraCartsChestMinecart implements OpenableGUI, IMinecart, IItemTransfer {

	private ItemStack storedItem = null;
	private int storedQty = 0;
	private NBTTagCompound compound = null;
	private Block dsu = GameRegistry.findBlock("MineFactoryReloaded", "machine.1");
	
	public EntityDSUCarts(World world) {
		super(world);
		this.setCartBlock(dsu);
	...
	public boolean doesCartMatchFilter(ItemStack stack, EntityMinecart cart) {
		...
		if (cart instanceof EntityDSUCarts && stack.getItem() == CartStack.getItem() && stack.getItemDamage() == 0) {
				return true;
		}
		return false;
	}
