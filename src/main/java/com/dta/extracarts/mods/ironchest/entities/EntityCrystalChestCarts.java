...
public class EntityCrystalChestCarts extends EntityExtraCartsChestMinecart implements OpenableGUI, IMinecart, IItemTransfer {
	
	private Block ironChest = Block.getBlockFromName("IronChest:BlockIronChest");
	
	public EntityCrystalChestCarts(World world) {
		super(world);
	...
	public boolean doesCartMatchFilter(ItemStack stack, EntityMinecart cart) {
		...
		if (cart instanceof EntityCrystalChestCarts && stack.getItem() == CartStack.getItem() && stack.getItemDamage() == 5) {
				return true;
		}
		return false;
	}
